...
namespace UnityEditor.XR.PicoVR
{
    public class PicoVRBuildProcessor : XRBuildHelper<PicoVRSettings>
    {
        public override string BuildSettingsKey { get {return "Unity.XR.PicoVR.Settings";} }
    }

    public static class PicoVRBuildTools
    {
        public static bool PicoVRLoaderPresentInSettingsForBuildTarget(BuildTargetGroup btg)
        {
            ...
            return loaders.Exists(loader => loader is PicoVRLoader);
        }

        public static PicoVRSettings GetSettings()
        {
...
#endif
            Debug.Log(settings.eyeRenderTextureResolution);
            return settings;
        }
    }

    [InitializeOnLoad]
    public static class PicoVREnterPlayModeSettingsCheck
    {
        static PicoVREnterPlayModeSettingsCheck()
        {
        ...
        private static void PlaymodeStateChangedEvent(PlayModeStateChange state)
        {
            ...
            {
                if (!PicoVRBuildTools.PicoVRLoaderPresentInSettingsForBuildTarget(BuildTargetGroup.Standalone))
                {
                    return;
                }
                ...
                {
                    Debug.LogError("D3D11 is currently the only graphics API compatible with the PicoVR XR Plugin on desktop platforms. Please change the preferred Graphics API setting in Player Settings.");
                }
            }
        }
    }

    internal class PicoVRPrebuildSettings : IPreprocessBuildWithReport
    {
        public int callbackOrder { get; }

        public void OnPreprocessBuild(BuildReport report)
        {
            if(!PicoVRBuildTools.PicoVRLoaderPresentInSettingsForBuildTarget(report.summary.platformGroup))
                return;
            ...
            if (report.summary.platformGroup == BuildTargetGroup.Android)
            {
                ...
                {
                    throw new BuildFailedException("Minimum API must be set to 21 or higher for PicoVR XR Plugin.");
            ...
            if (report.summary.platformGroup == BuildTargetGroup.Standalone)
            {
                ...
                //     GraphicsDeviceType.Direct3D11)
                // {
                    throw new BuildFailedException("D3D11 is currently the only graphics API compatible with the PicoVR XR Plugin on desktop platforms. Please change the Graphics API setting in Player Settings.");
                // }
            }
        }
    }
...
#if UNITY_ANDROID
    internal class PicoVRManifest : IPostGenerateGradleAndroidProject
    {
        ...
        {
            if(!PicoVRBuildTools.PicoVRLoaderPresentInSettingsForBuildTarget(BuildTargetGroup.Android))
               return;
            ...
            //if (!PicoVRBuildTools.GetSettings() || PicoVRBuildTools.GetSettings().V2Signing)
            //{
            //    nodePath = "/manifest";
